...
import type { ListaEmpresas, Projeto, StatusProjeto } from '../api/types';
...
export default function Projetos() {
  ...
  const criar = useMutation({
    mutationFn: async (payload: { nome: string; empresaId: string }) => {
  ...
  const mudarStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: StatusProjeto }) =>
      api.patch(`/projetos/${id}/status`, { status }),
  ...
  function aoSubmeter(e: FormEvent<HTMLFormElement>) {
    ...
    if (nome && empresaId) criar.mutate({ nome, empresaId });
  }
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--color-ink-soft)' }}>
            Implantações e entregas em andamento
          </p>
        </div>
        <button
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {projetos?.map((projeto) => (
                <tr key={projeto.id} className="border-b last:border-0" style={{ borderColor: 'var(--color-line)' }}>
                  <td className="px-4 py-3 font-medium text-ink">{projeto.nome}</td>
                  <td className="px-4 py-3" style={{ color: 'var(--color-ink-soft)' }}>{projeto.empresa?.nome}</td>
                  <td className="px-4 py-3">
                    <select
                      value={projeto.status}
                      onChange={(e) => mudarStatus.mutate({ id: projeto.id, status: e.target.value as StatusProjeto })}
